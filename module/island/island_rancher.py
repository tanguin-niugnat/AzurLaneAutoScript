...
class IslandRancher(Island, WarehouseOCR, LoginHandler):
    ...
    def run(self):
        ...
        time_vars = ['time_ranch1', 'time_ranch2', 'time_ranch3', 'time_ranch4',
                     'time_fish1', 'time_fish2', 'time_fish3']
        all_configs = [
            ('ISLAND_RANCH_POST1', 'time_ranch1'),
            ('ISLAND_RANCH_POST2', 'time_ranch2'),
            ('ISLAND_RANCH_POST3', 'time_ranch3'),
            ('ISLAND_RANCH_POST4', 'time_ranch4'),
            ('ISLAND_FISHERY_POST1', 'time_fish1'),
            ('ISLAND_FISHERY_POST2', 'time_fish2'),
            ('ISLAND_FISHERY_POST3', 'time_fish3')
        ]
        ...
        # 根据配置添加渔场岗位（值为0到3）
        cray_num = self.config.IslandFishery_Cray
        fishery_posts = ['ISLAND_FISHERY_POST1', 'ISLAND_FISHERY_POST2', 'ISLAND_FISHERY_POST3']
        for i in range(cray_num):
            post_id = fishery_posts[i]
            if post_id not in all_needs:
                all_needs.append(post_id)

        # 执行所有岗位
        if all_needs:
            logger.info(f"需要执行的岗位: {all_needs}")
